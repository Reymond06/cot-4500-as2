...
def main():
    ...
    newton_x = np.array([7.2, 7.4, 7.5, 7.6])
    newton_y = np.array([23.5492, 25.3913, 26.8224, 27.4589])
    diff_table = newton_forward(newton_x, newton_y)
    print_polynomial_approximations(newton_x, diff_table)
    x_to_interpolate = 7.3
    approximation = newton_forward_interpolation(newton_x, newton_y, x_to_interpolate)
    print(approximation,"\n")

    # Divided Difference
    divided_x=np.array([3.6,3.8,3.9])
    divided_y=np.array([1.675,1.436,1.318])
    divided_prime=np.array([-1.195,-1.188,-1.182])
    hermite_matrix = hermite(divided_x, divided_y, divided_prime)
    print(hermite_matrix[:6, :5])

    # Cubic Spline Interpolation
    cubic_x=np.array([2,5,8,10])
    cubic_y=np.array([3,5,7,9])
